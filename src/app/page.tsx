...
import { CampusHelperClient } from '@/components/CampusHelperClient';
...
export default async function Page() {
  ...
  // Auto-detect current active or default period
  ...
  // Default initial batch: 2CSE13 or first class
  const defaultBatch =
    timetable.classes.find((c) => c.name === '2CSE13') ||
    timetable.classes[0] ||
    null;

  const initialClassTimetable = defaultBatch
    ? getClassTimetable(timetable, defaultBatch.id)
    : null;
  ...
  const initialData: BootstrapData = {
    ...
    initialPeriodNum,
    initialAvailability,
    initialBatchId: defaultBatch?.id || '',
    initialClassTimetable,
  };

  return <CampusHelperClient initialData={initialData} />;
}
